...
class Testing {
    ...
    static func setupForTesting() {
        ...
        //load pins
        let hackGSU = (title: "GT Dorm",latitude: 33.7563920891773, longitude: -84.3890242522629, type: TPLocationType.Home)
        let iOSClub = (title: "iOS Club",latitude: 33.776732102728, longitude: -84.3958815877988, type: TPLocationType.Work)
        
        var locations = [TPLocation]()
        
        for testL in [hackGSU, iOSClub] {
            let newLocation = TPLocation()
            newLocation.title = testL.title
